...
- Potential-based pursuit shaping: the team reward's approach signal uses potential-based reward shaping (Ng, Harada & Russell 1999) instead of a stale-sighting distance delta. `_compute_team_reward()` defines a potential `F(s) = -POTENTIAL_SHAPING_ALPHA * min_bfs_distance(ghosts -> TRUE Pacman position)` (`POTENTIAL_SHAPING_ALPHA = 0.5` in `constant.py`) and emits the policy-invariant per-step term `F(s') - F(s)` (breakdown key `potential_shaping`). This gives a dense, correct gradient toward capture even while Pacman is unseen, replacing the former `DISTANCE_DECREASE/INCREASE` terms (now deprecated) that fired against a cached last-sighting position and created a stay-still local optimum. Exploration incentives `VALID_MOVE` (0.05) and `ENTER_RECENTLY_UNVISITED_TILE` (0.15) were raised so movement always beats inaction. The reward uses the privileged true Pacman position (the policy still acts only on each ghost's local 5x5 view). Validated: an IQL 60k run produced real captures (vs 0 in the failing baseline), confirming the stay-still trap is broken; late-training stability at this budget is tracked as a follow-up. Source: plan-000021 / research-000012.
