...
- Pluggable rewards: `PacManEnvironment` builds an immutable `RewardContext` after each transition and delegates all reward weights, shaping logic, and episode history to a strategy class loaded by `module:Class`. `CurrentTeamReward` preserves the potential-based pursuit signal `F(s') - F(s)` with alpha `0.5`. The `current_v3` variant (`PursuitFirstTeamReward`, plan-000028) is the pursuit-first rebalance: it zeros the anti-oscillation penalties and strengthens the potential gradient after a per-term diagnostic showed those penalties (not stalking) collapsed the V2 policy's return. Benchmark runs are separated by strategy ID and compared with paired capture-rate/time-to-capture evaluation rather than raw return scale; `eval_report.py` emits a `mean_reward_breakdown` JSON column so each term's contribution to the return is auditable. Source: plan-000028 / research-000027.
