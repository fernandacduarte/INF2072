...
- Win-rate evaluation harness: `custom_environment/eval.py` exposes a headless multi-episode mode (`--episodes N`, `--eval-epsilon`, `--win-rate-out`) that loads a checkpoint and reports the ghost win rate (captures vs Pacman survivals) plus an optional CSV. Outcome classification is shared with the renderer via the pure `classify_outcome(raw_env, step, max_steps)` helper (also used by `_build_final_result`), so harness and on-screen result never diverge. Because the environment is deterministic, episodes are varied with seeded per-ghost epsilon-greedy sampling (`--eval-epsilon`, default 0.05). **Measured IQL baseline (plan-000008): 0% win rate at 60k and 300k frames with a flat training reward curve** — a coordination ceiling for the independent-learner baseline against the defense-first Pacman; VDN/QMIX are the coordination path expected to win. Reward magnitudes were retuned (capture `+30`, distance `±0.5`, visibility `+0.3`, trimmed exploration bonuses; signs unchanged). Source: plan-000008.
